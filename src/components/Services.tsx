      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {services.map((service) => (
            <div
              key={service.title}
              className="group p-6 rounded-xl border border-[#e2e8f0] hover:border-[#1e3a5f] hover:shadow-lg transition-all duration-300"
            >
              ...
              <div className="flex flex-wrap gap-1.5">
                {service.tags.map((tag) => (
                  <span
                    key={tag}
                    className="text-xs bg-[#f1f5f9] text-[#64748b] px-2 py-0.5 rounded-full"
                  >
                    {tag}
                  </span>
                ))}
